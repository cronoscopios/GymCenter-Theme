...
if ( ! function_exists( 'gymclub_setup' ) ) :
	...
	function gymclub_setup() {
		...
		add_image_size( 'blog-single', 750, 320, array( 'left', 'top' ), true);
		add_image_size( 'blog-index', 750, 310, true );
		add_image_size( 'single-entrenador', 380, 550, array( 'left', 'top' ), true);
		add_image_size( 'entrenador-galeria', 400, 300, array( 'left', 'top' ), true);
...
function gymclub_widgets_init() {
	...
	register_sidebar( array(
		...
		'id'            => 'footer-3',
		'description'   => esc_html__( 'Add widgets here footer zone 3', 'gymclub' ),
		'before_widget' => '<section id="%1$s" class="widget %2$s">',
		'after_widget'  => '</section>',
		'before_title'  => '<h2 class="widget-title">',
		'after_title'   => '</h2>',
...
function gymclub_scripts() {
	...
	wp_enqueue_style('iconos_fontawesome', 'https://use.fontawesome.com/releases/v5.7.2/css/all.css', false );
 ...
 * Custom form.
 */
require get_template_directory() . '/inc/custom_form.php';


/**
 * Custom Post_type.
 */
require get_template_directory() . '/inc/entrenadores.php';
